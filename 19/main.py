...
def part1(lines):
    ...
    for x in range(0, 50):
        for y in range(0, 50):
            ...
            if result.outputStream[0] == 1:
                count += 1

    return f"The amount of spaces affected by tractor beam is {count}."
...
def part2(lines):
    ...
    while True:
        # find the left edge beam
        left_edge_found = False
        while not left_edge_found:
            computer = ComputerState(lines[0])
            result = computer.run([x, y])
            if result.outputStream[0] == 1:
        ...
        computer = ComputerState(lines[0])
        result = computer.run([x+99, y-99])

        if result.outputStream[0] == 1:
            return f"Coordinate where Santa sleigh will fit is {x*10000+y-99}."
        else:
            y += 1
